	...
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if ((res == null) || (res == DefaultDataControllerStream))
                	res = ControllerConfigurationUtility.GetResourceStream(String.Format("VSM.controllers.{0}.xml", controller), String.Format("VSM.{0}.xml", controller));
                if (res == null)
                {
        ...
        protected virtual bool ConfigureCommand(DbCommand command, ViewPage page, CommandConfigurationType commandConfiguration, FieldValue[] values)
        {
            ...
            if (command == null)
            	return true;
            if (values != null)
            	VSM.Security.EventTracker.EnsureTrackingFields(page, _config);
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(VSM.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("VSM.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("VSM.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = VSM.Data.Controller.CreateConfigurationInstance(t, controller);
